...
class IF(Pipe):
    ...
    def compute(self):
        ...
        self.ntaddr = Pipe.cpu.adder_pcplus4.op(self.pc, 4)
        # Compute PC + 4 using an adder
        if self.c_br_type in [ BR_NE, BR_EQ, BR_GE, BR_GEU, BR_LT, BR_LTU, BR_J ]:
            self.pcplus4    = Pipe.cpu.adder_brtarget.op(self.pc, self.imm)
        else:
            self.pcplus4    = self.ntaddr
...
class ID(Pipe):
    ...
    def compute(self):
        ...
        self.pc_sel         =   PC_BRJMP    if (EX.reg_c_br_type == BR_NE  and Pipe.EX.alu_out) or          \
...
class EX(Pipe):
    ...
    reg_ntaddr          = WORD(0)           # EX.reg_pcplus4
